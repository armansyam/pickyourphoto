...
export async function getVendorDriveClient(vendorId) {
  try {
    ...
    oauth2Client.setCredentials({ refresh_token: rawToken });
...
/**
 * Membuat Sub-Folder baru langsung di Google Drive milik Vendor (BYOS External Drive)
 */
export async function createVendorExternalSubFolder(vendorId, parentFolderId = 'root', folderName) {
  const drive = await getVendorDriveClient(vendorId);
  if (!drive) throw new Error('Akun Google Drive Vendor belum terhubung atau token kedaluwarsa.');

  const parentId = (!parentFolderId || parentFolderId === 'root') ? 'root' : parentFolderId;
