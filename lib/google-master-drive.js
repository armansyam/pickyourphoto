...
export async function fetchFolderFilesMasterOAuth(folderId, categoryName = '') {
  ...
  do {
    ...
    // 2. Process sub-folders recursively
    if (!categoryName) {
      const subFolders = pageFiles.filter(file => file.mimeType === 'application/vnd.google-apps.folder');
      for (const sf of subFolders) {
        try {
          const sfFiles = await fetchFolderFilesMasterOAuth(sf.id, sf.name);
