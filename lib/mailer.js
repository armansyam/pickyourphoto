...
export async function sendVendorRejectionEmail(vendor, plan, reason = 'Pendaftaran ditolak oleh administrator.') {
    ...
    const mailOptions = {
        from: `"${config.fromName}" <${config.email}>`,
        to: vendor.email,
        subject: `Informasi Pendaftaran Pick Your Photo — ${vendor.name}`,
        html: `
            <div style="font-family: sans-serif; background: #09090b; color: #ffffff; padding: 30px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); max-width: 600px; margin: 0 auto;">
                <h2 style="color: #f87171;">Informasi Status Pendaftaran</h2>
                <p>Halo <strong>${vendor.name}</strong>,</p>
                <p>Terima kasih atas minat Anda bergabung dengan <strong>${config.fromName}</strong>.</p>
                <p>Setelah kami tinjau, pendaftaran Anda saat ini <strong>belum dapat kami proses</strong> dengan keterangan berikut:</p>
                <div style="background: rgba(248,113,113,0.1); border: 1px solid rgba(248,113,113,0.3); border-radius: 8px; padding: 16px; margin: 16px 0;">
                    <p style="margin: 0; color: #f87171;">${reason}</p>
                </div>
                <p>Jika Anda memiliki pertanyaan atau ingin mendaftar ulang, silakan hubungi administrator atau kunjungi halaman pendaftaran kembali.</p>
                <p style="color: #a1a1aa; font-size: 12px; margin-top: 24px;">Email ini dikirim secara otomatis. Waktu: ${new Date().toLocaleString('id-ID')}</p>
            </div>
        `
    };
