...
export async function createIPaymuTransaction({ orderId, amount, vendorName, vendorEmail, vendorPhone, planName, config }) {
  ...
  // Expiration in minutes from Admin settings
  const expiryMinutes = Number(config.qrisExpirationMinutes) > 0 ? Number(config.qrisExpirationMinutes) : 5;

  const dynamicBaseUrl = config.baseUrl || process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000';
  const notifyUrl = config.notifyUrl || `${dynamicBaseUrl}/api/payment/notification`;
  const returnUrl = config.returnUrl || `${dynamicBaseUrl}/dashboard`;
  const cancelUrl = config.cancelUrl || `${dynamicBaseUrl}/register`;
  ...
  let qrImage = data.QrImage || data.qrImage || '';
  const qrString = data.QrString || data.qrString || '';

  // If qrImage is an HTML wrapper URL from IPaymu (e.g. https://sandbox.ipaymu.com/qris-basic/...),
  // fetch and extract the real data:image/png;base64,... URI so <img> tags render natively in all browsers
  if (qrImage && (qrImage.startsWith('http://') || qrImage.startsWith('https://'))) {
    try {
      const html = await fetch(qrImage).then(r => r.text());
      const match = html.match(/src=["'](data:image\/[^"']+)["']/i);
      if (match && match[1]) {
        qrImage = match[1];
      }
    } catch (err) {
      console.warn('[IPaymu QR Extraction Warning]:', err.message);
    }
  }
