...
export async function createIPaymuTransaction({ orderId, amount, vendorName, vendorEmail, vendorPhone, planName, config }) {
  ...
  // Expiration in minutes (IPaymu v2 supports expiredType: 'minutes')
  const expiryMinutes = config.qrisExpirationMinutes && config.qrisExpirationMinutes > 0
    ? parseInt(config.qrisExpirationMinutes, 10)
    : 15;
