 ...
 * Uses Snap API — token rendered via snap.embed() inline (no popup).
 */
export async function createMidtransTransaction({ orderId, amount, vendorName, vendorEmail, vendorPhone, planName, config }) {
  ...
  if (!serverKey) throw new Error('Midtrans Server Key belum diatur di Admin Panel.');

  const authHeader = 'Basic ' + Buffer.from(serverKey + ':').toString('base64');
  const expiryDuration = config.qrisExpirationMinutes && config.qrisExpirationMinutes > 0
    ? config.qrisExpirationMinutes : 15;

  const payload = {
    transaction_details: { order_id: orderId, gross_amount: Math.round(amount) },
    customer_details: { first_name: vendorName || 'Vendor', email: vendorEmail, phone: vendorPhone || '' },
    item_details: [{ id: orderId, price: Math.round(amount), quantity: 1, name: `Berlangganan ${planName || 'Paket SaaS'}`.substring(0, 50) }],
    expiry: { unit: 'minute', duration: expiryDuration },
  };
  ...
  if (!response.ok) throw new Error(json.error_messages ? json.error_messages.join(', ') : 'Gagal membuat transaksi Midtrans');

  return { token: json.token, redirectUrl: json.redirect_url, raw: json };
}
