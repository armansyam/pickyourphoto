...
export function getPaymentGatewayConfig() {
  try {
    ...
    return {
      ...
      qrisExpirationMinutes: parseInt(settings.qris_expiration_minutes || '5', 10),
    };
  } catch (err) {
 ...
 */
export async function createPayment({ orderId, amount, vendorName, vendorEmail, vendorPhone, planName, baseUrl, notifyUrl, returnUrl, cancelUrl }) {
  const config = getPaymentGatewayConfig();
  if (!config.enabled) {
    throw new Error('Fitur Payment Gateway saat ini sedang dinonaktifkan oleh Admin.');
  }

  // Bind dynamic URLs based on current request host / environment
  if (baseUrl) config.baseUrl = baseUrl;
  if (notifyUrl) config.notifyUrl = notifyUrl;
  if (returnUrl) config.returnUrl = returnUrl;
