...
export async function createDuitkuTransaction({ orderId, amount, vendorName, vendorEmail, vendorPhone, planName, config }) {
  ...
  const baseUrlHost = process.env.NEXT_PUBLIC_BASE_URL || 'https://photota.id';
  const callbackUrl = config.notifyUrl || `${baseUrlHost}/api/payment/notification`;
  const returnUrl = config.returnUrl || `${baseUrlHost}/dashboard`;
