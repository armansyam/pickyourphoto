...
function initDb() {
  ...
  `);
    ...
    try { masterDb.exec("ALTER TABLE vendors ADD COLUMN pendingAddonPlanId TEXT;"); } catch (_) {}
    try { masterDb.exec("ALTER TABLE vendors ADD COLUMN pendingAddonQuotaBytes INTEGER DEFAULT 0;"); } catch (_) {}
    try { masterDb.exec("ALTER TABLE vendors ADD COLUMN portfolioDriveUrl TEXT;"); } catch (_) {}

    // Self-healing columns for payment_sessions & payment_transactions
    try { masterDb.exec("ALTER TABLE payment_sessions ADD COLUMN addonPlanId TEXT;"); } catch (_) {}
    try { masterDb.exec("ALTER TABLE payment_sessions ADD COLUMN transactionType TEXT DEFAULT 'plan';"); } catch (_) {}
    try { masterDb.exec("ALTER TABLE payment_transactions ADD COLUMN addonPlanId TEXT;"); } catch (_) {}
    try { masterDb.exec("ALTER TABLE payment_transactions ADD COLUMN addonQuotaBytes INTEGER DEFAULT 0;"); } catch (_) {}
    try { masterDb.exec("ALTER TABLE payment_transactions ADD COLUMN transactionType TEXT DEFAULT 'plan';"); } catch (_) {}
