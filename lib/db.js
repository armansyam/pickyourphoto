  ...
  `);
    ...
    try {
        ...
        if (!adminEmail || !adminPass) {
            ...
            const mainAdmin = db.prepare("SELECT id, email, password FROM admins ORDER BY id ASC LIMIT 1").get();

            if (!mainAdmin) {
                const hashedPassword = bcrypt.hashSync(adminPass, 10);
                db.prepare("INSERT INTO admins (name, email, password, role, status) VALUES ('System Owner', ?, ?, 'admin', 'active')").run(
                    cleanEnvEmail,
                    hashedPassword
                );
                console.log(`[Database Init] Berhasil melakukan seed akun admin baru di tabel admins: ${cleanEnvEmail}`);
            } else {
                ...
                if (isEmailDiff || isPassDiff) {
                    const hashedPassword = bcrypt.hashSync(adminPass, 10);
                    db.prepare("UPDATE admins SET email = ?, password = ? WHERE id = ?").run(
                        cleanEnvEmail,
                        hashedPassword,
                        mainAdmin.id
                    );
                    console.log(`[Database Init] Berhasil sinkronisasi Master Recovery kredensial admin dari .env.local: ${cleanEnvEmail}`);
                }
            }
        }
    } catch (err) {
