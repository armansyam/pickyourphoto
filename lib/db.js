...
try {
    ...
    console.warn("[Database Warning] Gagal mengaktifkan WAL Mode/busy_timeout (mungkin sedang dikunci oleh proses lain):", err.message);
}
...
function initDb() {
  ...
  `);
    ...
    // Seed default plans if empty, or update existing default plans to new optimized WebP pricing & limits
    try {
        ...
        if (planCount === 0) {
            ...
            insertPlan.run('Trial Limit', 3, 0, 99999, 50, 1, 'limit', 51200);
            insertPlan.run('Starter', 5, 49000, 99999, 300, 30, 'limit', 51200);
            insertPlan.run('Pro', 15, 99000, 99999, 600, 30, 'limit', 51200);
            insertPlan.run('Business', 35, 199000, 99999, 1200, 30, 'limit', 51200);

            // Storage-based default plans (Free Trial set to 1 day)
            insertPlan.run('Trial Storage', 99999, 0, 99999, 99999, 1, 'storage', 100);
            insertPlan.run('Lite', 99999, 35000, 99999, 99999, 30, 'storage', 1024);
            insertPlan.run('Pro Storage', 99999, 75000, 99999, 99999, 30, 'storage', 3072);
            insertPlan.run('Studio Storage', 99999, 149000, 99999, 99999, 30, 'storage', 10240);

            console.log('Seeded 8 default plans with optimized WebP tier pricing & limits.');
        } else {
            // Step 1: Update Storage plans first to free up 'Pro' and 'Business' names
            db.prepare("UPDATE plans SET name = 'Lite', price = 35000, maxStorageMB = 1024 WHERE name = 'Base' OR name = 'Lite'").run();
            db.prepare("UPDATE plans SET name = 'Pro Storage', price = 75000, maxStorageMB = 3072 WHERE (name = 'Pro' AND planType = 'storage') OR name = 'Pro Storage'").run();
            db.prepare("UPDATE plans SET name = 'Studio Storage', price = 149000, maxStorageMB = 10240 WHERE (name = 'Business' AND planType = 'storage') OR name = 'Studio Storage'").run();

            // Step 2: Update Limit plans
            db.prepare("UPDATE plans SET name = 'Starter', price = 49000, maxPhotosPerProject = 300 WHERE name = 'Tier 1' OR name = 'Starter'").run();
            db.prepare("UPDATE plans SET name = 'Pro', price = 99000, maxPhotosPerProject = 600 WHERE name = 'Tier 2' OR (name = 'Pro' AND planType = 'limit')").run();
            db.prepare("UPDATE plans SET name = 'Business', price = 199000, maxPhotosPerProject = 1200 WHERE name = 'Tier 3' OR (name = 'Business' AND planType = 'limit')").run();
        }
    } catch (err) {
