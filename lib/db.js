...
function initDb() {
  ...
  `);
    ...
    // Seed default 4 Paket Add-On Storage Dinamis (jika belum ada)
    try {
      ...
      if (!checkAddon || checkAddon.count === 0) {
        const insertAddon = db.prepare(`
          ...
          VALUES (?, ?, ?, ?, 'active', ?)
        `);
        insertAddon.run('addon_25gb', 'Add-On Storage 25 GB', 26843545600, 49000, 1);
        insertAddon.run('addon_50gb', 'Add-On Storage 50 GB', 53687091200, 89000, 2);
        insertAddon.run('addon_100gb', 'Add-On Storage 100 GB', 107374182400, 149000, 3);
        insertAddon.run('addon_200gb', 'Add-On Storage 200 GB', 214748364800, 249000, 4);
        console.log('[Database Init] Seeded default 4 Dynamic Add-On Storage Plans cleanly.');
      }
    } catch (err) {
  ...
  `);
    ...
    // Safe column migrations for Add-On Payments
...
if (!isNextBuild) {
    ...
    setInterval(() => {
        try {
            ...
            // 5. Auto-delete archived/cancelled/rejected vendors older than 7 days (7-day archive cleanup)
            ...
            if (expiredSessions.changes > 0 || archivedVendors.changes > 0 || deletedLeads.changes > 0 || deletedArchived.changes > 0) {
                console.log(`[DB Cleanup] Sessions expired: ${expiredSessions.changes}, Archived: ${archivedVendors.changes}, Leads cleaned: ${deletedLeads.changes}, Archived purged: ${deletedArchived.changes}`);
            }
