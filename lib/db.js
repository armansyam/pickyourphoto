...
function initDb() {
  ...
  `);
    ...
    try {
        ...
        // 2. Seed/Sync kredensial admin dari environment
        ...
        if (!adminEmail || !adminPass) {
            ...
            const adminExists = db.prepare("SELECT id, email, password FROM admins WHERE email = ?").get(adminEmail.toLowerCase().trim());
            if (!adminExists) {
                ...
                console.log(`[Database Init] Berhasil melakukan seed akun admin baru di tabel admins: ${adminEmail}`);
            } else {
                ...
                if (isPasswordDifferent) {
                    ...
                    console.log(`[Database Init] Berhasil sinkronisasi password admin di tabel admins: ${adminEmail}`);
                }
            }
        }
    } catch (err) {
