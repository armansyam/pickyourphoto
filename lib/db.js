...
function initDb() {
  ...
  `);
    ...
    try {
        // Clean up legacy Trial & Storage-based plans from plans table
        db.exec("DELETE FROM plans WHERE name LIKE '%Trial%' OR planType = 'storage' OR name LIKE '%Storage%' OR name = 'Base' OR name = 'Tier 1' OR name = 'Tier 2' OR name = 'Tier 3' OR name = 'Unlimited Master Plan';");
        // Ensure all plans have activePeriodDays = 30, projectExpireDays = 30, maxPhotosPerProject = 0 (Unlimited Photos), maxStorageMB = 0 (Zero Storage)
        db.exec("UPDATE plans SET activePeriodDays = 30, projectExpireDays = 30, maxPhotosPerProject = 0, maxStorageMB = 0;");
        ...
        if (!checkPlans || checkPlans.count === 0) {
            const insertPlan = db.prepare(`
                INSERT INTO plans (name, maxProjects, price, projectExpireDays, maxPhotosPerProject, activePeriodDays, status, planType, maxStorageMB, allowCustomLogo) 
                VALUES (?, ?, ?, ?, ?, ?, 'active', 'limit', 0, ?)
            `);

            insertPlan.run('Starter Plan', 5, 49000, 30, 0, 30, 0);
            insertPlan.run('Pro Studio Plan', 20, 129000, 30, 0, 30, 1);
            insertPlan.run('Business Studio Plan', 50, 249000, 30, 0, 30, 1);
            console.log('Seeded 3 ideal SaaS Plans cleanly with 30-day billing, 30-day gallery retention & Unlimited Photos.');
        }
...
if (!isNextBuild) {
    ...
    setInterval(() => {
        try {
            ...
            // 3. Auto-delete archive entries older than 7 days (permanent cleanup)
            const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
            const deletedVendors = db.prepare(`
                DELETE FROM vendors
                WHERE status IN ('expired_draft', 'cancelled', 'rejected')
                  AND archivedAt IS NOT NULL
                  AND archivedAt < ?
                  AND role != 'admin'
            `).run(sevenDaysAgo);

            if (expiredSessions.changes > 0 || archivedVendors.changes > 0 || deletedVendors.changes > 0) {
                console.log(`[DB Cleanup] Sessions expired: ${expiredSessions.changes}, Vendors archived: ${archivedVendors.changes}, Vendors deleted: ${deletedVendors.changes}`);
            }
        } catch (cleanupErr) {
