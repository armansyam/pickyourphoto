...
function initDb() {
  ...
  `);
    ...
    try {
        ...
        // Ensure all plans have maxPhotosPerProject = 0 (Unlimited Photos) & projectExpireDays = 99999 (Permanent Gallery Retention)
        db.exec("UPDATE plans SET maxPhotosPerProject = 0 WHERE maxPhotosPerProject > 0;");
        db.exec("UPDATE plans SET projectExpireDays = 99999 WHERE projectExpireDays < 99999;");
        ...
        if (!checkPlans || checkPlans.count === 0) {
            const insertPlan = db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, 'active', 'limit', 51200, ?)
            `);

            insertPlan.run('Starter Plan', 5, 49000, 99999, 0, 30, 0);
            insertPlan.run('Pro Studio Plan', 20, 129000, 99999, 0, 180, 1);
            insertPlan.run('Business Studio Plan', 50, 249000, 99999, 0, 365, 1);
            console.log('Seeded 3 ideal SaaS Plans cleanly with Unlimited Photos & Permanent Gallery Retention.');
        }
    } catch (err) {
  ...
  `);
    ...
    try {
        ...
        console.error("Migrasi trial_galleries logoUrl error:", e);
    }
