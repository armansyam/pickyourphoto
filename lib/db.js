...
if (process.env.NODE_ENV === 'production') {
    ...
    if (!globalThis._sqliteDb) {
        ...
        try {
            ...
            globalThis._sqliteDb.pragma('synchronous = NORMAL');
            globalThis._sqliteDb.pragma('busy_timeout = 10000');
            globalThis._sqliteDb.pragma('temp_store = MEMORY');
...
function initDb() {
  ...
  `);
    ...
    // Migrasi: Hard Purge otomatis (vendor expired melewati grace period)
    try { db.exec("ALTER TABLE system_settings ADD COLUMN enable_auto_purge INTEGER NOT NULL DEFAULT 1;"); } catch (e) {}
    try { db.exec("ALTER TABLE system_settings ADD COLUMN last_hard_purge_at TEXT;"); } catch (e) {}
  ...
  `);
    ...
    try {
        ...
        try {
            db.prepare("UPDATE plans SET maxProjects = 5, price = 49000, allowCustomLogo = 0, allowRawSelector = 0 WHERE name LIKE '%Starter%'").run();
            db.prepare("UPDATE plans SET maxProjects = 20, price = 129000, allowCustomLogo = 1, allowRawSelector = 0 WHERE name LIKE '%Pro Studio%'").run();
    ...
    try {
        ...
        // Fallback cerdas: Baca langsung berkas .env.local / .env jika process.env belum diinjeksi
        ...
        if (!adminEmail || !adminPass) {
            console.warn("[Database Init] ADMIN_EMAIL atau ADMIN_PASSWORD belum diatur di environment. Seeding superadmin ditangguhkan.");
        } else {
  ...
  `);
    ...
    try { db.exec("ALTER TABLE subscription_requests ADD COLUMN isBundlePromo INTEGER DEFAULT 0;"); } catch (_) {}
    try { db.exec("ALTER TABLE subscription_requests ADD COLUMN bundleAddonName TEXT;"); } catch (_) {}
    try { db.exec("ALTER TABLE subscription_requests ADD COLUMN bundleAddonType TEXT;"); } catch (_) {}
    try { db.exec("ALTER TABLE subscription_requests ADD COLUMN bundleAddonValue INTEGER DEFAULT 0;"); } catch (_) {}
...
if (!isNextBuild) {
    ...
    setInterval(() => {
        try {
            ...
            // 6. Autonomous Hard Purge Cron (Runs every 24 hours based on system_settings.enable_auto_purge)
            // Menghapus data DB vendor yang sudah melewati grace period secara otomatis
            try {
                const purgeSettings = db.prepare("SELECT enable_auto_purge, last_hard_purge_at FROM system_settings WHERE id = 1").get();
                const autoPurgeEnabled = !purgeSettings || purgeSettings.enable_auto_purge !== 0; // default ON (1)
                if (autoPurgeEnabled) {
                    const PURGE_INTERVAL_MS = 24 * 60 * 60 * 1000; // 24 jam
                    const lastPurgeMs = purgeSettings?.last_hard_purge_at ? new Date(purgeSettings.last_hard_purge_at).getTime() : 0;
                    if (Date.now() - lastPurgeMs > PURGE_INTERVAL_MS) {
                        // Cari vendor yang melewati grace period
                        ...
                        const vendorsToPurge = db.prepare(`
                            SELECT id, email, name FROM vendors 
                            WHERE expiresAt IS NOT NULL AND status != 'suspended' AND status != 'purged'
                        `).all().filter(v => {
                            const expMs = new Date(v.expiresAt).getTime();
                            return expMs > 0 && nowMs > (expMs + graceMs);
                        });

                        if (vendorsToPurge.length > 0) {
                            console.log(`[Auto Purge Cron] ${vendorsToPurge.length} vendor melewati grace period. Memulai hard purge DB...`);
                            ...
                            for (const vendor of vendorsToPurge) {
                                try {
                                    db.transaction(() => {
                                        ...
                                        db.prepare("UPDATE vendors SET status = 'suspended', usedStorageBytes = 0 WHERE id = ?").run(vendor.id);
                                    })();
                                    console.log(`[Auto Purge Cron] Vendor ID ${vendor.id} (${vendor.email}) — DB records cleaned.`);
                                } catch (vendorPurgeErr) {
                                    console.error(`[Auto Purge Cron] Gagal purge vendor ${vendor.id}:`, vendorPurgeErr.message);
                                }
                            }

                            // Catat waktu purge terakhir ke DB
                            db.prepare("UPDATE system_settings SET last_hard_purge_at = datetime('now') WHERE id = 1").run();
                            console.log(`[Auto Purge Cron] Selesai. ${vendorsToPurge.length} vendor di-suspend dan DB dibersihkan.`);
                        } else {
                            // Tidak ada vendor yang perlu dipurge — tetap catat timestamp agar tidak cek ulang tiap menit
                            db.prepare("UPDATE system_settings SET last_hard_purge_at = datetime('now') WHERE id = 1").run();
                        }
                    }
                }
            } catch (purgeErr) {
                console.error('[Auto Purge Cron Error]:', purgeErr.message);
            }
        } catch (cleanupErr) {
